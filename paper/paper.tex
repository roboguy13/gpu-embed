...
is an example that demonstrates why this is more useful than if it *fully* deconstructed
each type into \ttt{Either}s and \ttt{(,)}:
...
If \ttt{ERep ComplexPair} was fully deconstructed into
\ttt{((Double, Double), (Double, Double))}, we would need a \ttt{Num}
instance for \ttt{(Double, Double)}.
What we really want is to use the \ttt{Num} instance that already
exists for \ttt{Complex Double}, and this is what preserving this type
...
\begin{tcolorbox}
  \textbf{TODO}: Should we use a single nested pattern match in the above block? Both
...
\begin{tcolorbox}
  \textbf{TODO}: Expand the explanation in this section
\end{tcolorbox}
...
\begin{tcolorbox}
  \textbf{TODO}: Should we use Oxford brackets here? Maybe a bit unusual, but we are operating syntactically and mapping from \ttt{a} to \ttt{E a} (could \ttt{E a} be considered an "intermediary" semantic domain?).
\end{tcolorbox}
\vspace*{-0.6cm}
\begin{align*}
