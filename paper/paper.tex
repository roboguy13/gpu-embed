...
the lambda.
\begin{todo}
  Add brief summary of relationship to lambdas
\end{todo}
...
\begin{todo}
  Cite "Trampolined Style" (S. E. Gantz, D. P. Friedman, M. Ward; 1999)
\end{todo}

In the technique presented here, we do not need to allocate closures and we
actually do not to use higher-order functions at all.
...
\begin{todo}
  Cite "Rewriting a Shallow DSL using a GHC Compiler Extension"
  (2017; Grebe, Young, Gill)
\end{todo}
...
The standard semantics for the EDSL is given by \ttt{abs}.
...
\begin{todo}
  Cite and compare/contrast with pattern matching technique described in section
  3.3 of "Unembedding Domain-Specific Languages" (2009; Atkey, Lindley, Yallop)
\end{todo}
