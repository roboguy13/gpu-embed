...
\begin{abstract}
  ...
  Capturing control flow in an embedded domain specific language is crucial for
  writing programs in a "natural" style. This enables the programmer to use the
  constructs that are already builtin to the base language, such as pattern
  matching and recursion.

  In addition to control flow, when these features are reified, the data flow in
  a deeply embedded domain specific langauge also falls out naturally.
...
In embedded domain specific languages which do not capture control flow
constructs, workarounds must be employed which will be less familiar
than the builtin control flow mechanisms of the host language. When
control flow mechanisms are represented in the embedded language,
a much more natural style of programming is offered to the programmer.

Languages with reification of code fragments allow
...
% 
  Haskell in particular has been used for many embedded code generators, such
  as Lava, SunRoof, Conal work, CCC, AD, ... (finally tagless) diagrams? Feldspar (note: Feldspar has some form of \ttt{switch} primitive)?
...
for capturing the useful abstraction of pattern matching. This is accomplished
using a deeply embedded representation of the pattern match construct.

%Here is why the world will be a better place

With this new support, the Haskell that we capture for expressing
embedded-based solutions can be cleaner, more Haskell idiomatic,
