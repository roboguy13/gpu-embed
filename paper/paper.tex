...
% Say what problem is
  ...
  deeply embedding Domain Specific Languages,
  clunkly workarounds must be employed which will be less familiar
  than the builtin control flow mechanisms of the host language.  
% Say why it is an interesting problem
  However, capturing control flow in an embedded domain specific language is crucial for
  writing programs in a natural style, and enables the programmer to use the
...
% Here is why the world will be a better place
  ...
  deeply embedded domain specific languges can be cleaner, more Haskell idiomatic,
