...
has needed to be reimplemented each time. In this paper, we descibe a \textit{reusable}
...
% In the flowchart given in Figure~\ref{fig:CorePlugin}, the names in parentheses
% in a node refer to the names of transformations given in the rewrite rules in
% Figure~\ref{fig:Rewrites}. The control flow of the plugin is described by the
% flowchart and the rewrites performed when control reaches a node is given by the list of
% rule names in parentheses in that node.
...
%     % An example usage of this mechanism is given in Section
