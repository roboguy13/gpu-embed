...
A major benefit of the Haskell language is that you can often derive an
implementation from a specification, either partially or fully. When it is not
possible to fully derive an implementation from a specification, it is often
easier to check an implementation against a specification, due in part to the
management of side effects in Haskell. This is more difficult to do in
imperative languages like C or CUDA (as they provide no restrictions on where
side effects can occur).

Using Haskell EDSLs that allow the use of powerful language features like
pattern matching, these advantages can be brought to other domains where it is
currently either difficult or impossible to use Haskell.

\begin{todo}
  Cite examples. Conal Elliott's last two papers (the one about convolutions and
  the latest one about automatic differentiation), would probably be good
  examples of this in Haskell.

  A paper which describes a Haskell implementation of a specification given as
  an operational semantics would also be a good example.
\end{todo}
